 ...
 */
export async function extractCorpusHistory(file: File): Promise<CorpusCommitInput[] | null> {
    ...
    try {
        ...
        return log.map(entry => ({
            ...
            branch: branch ?? null,
            committedAt: entry.commit.author ? new Date(entry.commit.author.timestamp * 1000).toISOString() : null,
        }))
    } catch (error) {
 ...
 */
export async function fetchHuggingFaceHistory(url: string): Promise<CorpusCommitInput[] | null> {
    ...
    try {
        ...
        return entries.map(entry => ({
            sha: entry.id,
            message: [entry.title, entry.message?.trim()].filter(Boolean).join("\n\n"),
