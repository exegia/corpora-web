import type { ContentResponse, CorpusIndex, CorpusNode, IndexItem, SectionEntry } from "@/lib/api"
import type { CorpusDocument, CorpusSection } from "./types"
import type { Lemma, NodeTypeStat, StructureNode } from "@/components/corpus/detail/types"
...
export function findIndexItem(index: CorpusIndex, title: string | null): IndexItem | null {
    const items = index.sections?.items
...
export function structureRootFromIndex(document: CorpusDocument, index: CorpusIndex): StructureNode | null {
...
export function structureNodeFromSection(item: SectionEntry, fallbackType: string): StructureNode {
    return {
        id: item.ref || item.title,
...
export function passagesToNodes(content: ContentResponse, parent: StructureNode): StructureNode[] {
...
export function nodeTypeStatsFromIndex(index: CorpusIndex): NodeTypeStat[] {
    ...
    const slot = types.reduce((best, row) => (row.count > best.count ? row : best))
...
function featureString(features: Record<string, unknown>, keys: string[]): string | undefined {
 ...
 * is itself the token.
 */
export function slotForToken(node: CorpusNode, wordIndex: number): number | null {
...
export function lemmaFromNode(node: CorpusNode, form: string): Lemma {
    const lemma = featureString(node.features, ["lemma", "lex", "word", "form"]) ?? form
