...
import { LicenceTextDialog } from "@/components/project/licence-text-dialog"
import type { CatalogLicence } from "@/lib/licenses"
import { type AttachedLicense, type ProjectDetail } from "@/lib/projects"
import { useFetcher } from "react-router"
import { Check, Eye, Scale, Trash, X } from "lucide-react"
...
function LicenseIdentity({
    ...
    /** Second line — agreement provenance once agreed, else what the licence is. */
    meta: string
}) {
    return (
...
/** What the licence is, for an attachment with no agreement to describe yet. */
                    ...
                    <div className="flex shrink-0 items-center gap-1">
                        ...
                        <agreeFetcher.Form method="post">
                            ...
                            <input
                                type="hidden"
                                name="agreedByUserId"
                                value={agreedByUserId}
                            />
                            <Button
                                type="submit"
                                size="sm"
                                variant="outline"
                                disabled={agreeFetcher.state !== "idle"}
                            >
                                Review &amp; Agree
                            </Button>
                        </agreeFetcher.Form>
        ...
        <li className="group/row flex items-center gap-3 py-2">
            ...
            */}
            <div className="relative z-10 grid shrink-0 items-center justify-items-end">
                ...
                <div className="pointer-events-none flex [grid-area:1/1] flex-col items-end gap-0.5 transition-opacity group-focus-within/row:opacity-0 group-hover/row:opacity-0">
                    <p className="flex items-center gap-1.5 font-medium text-emerald-700 text-xs dark:text-emerald-400">
                        <Check
                            aria-hidden="true"
                    ...
                    <p className="whitespace-nowrap text-muted-foreground text-xs">
                        {formatDate(license.agreedAt)} by{" "}
                    ...
                    {project.licenses.length === 0 ? (
                        <Empty className="py-8 md:py-10">
                            <EmptyHeader>
                                ...
                                <EmptyDescription>
                                    A licence is required before the project can go to
                                    review — attach one from the catalog.
                                </EmptyDescription>
                            </EmptyHeader>
                        </Empty>
                    ) : (
