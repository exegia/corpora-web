...
export function Classify({ open, onOpenChange, current }: ClassifyDialogProps) {
    ...
    const languageItems = (languageOptionsFor(type) as LanguageType[]).map(language => ({
        value: language,
        label: language.charAt(0).toUpperCase() + language.slice(1),
    }))
    const selectedLanguageItems = languageItems.filter(item => languages.includes(item.value))
    const incomplete = (needsLanguage && languages.length === 0) || (needsCategory && !category)
    ...
    const submit = () => {
        ...
        fetcher.submit({ intent: "classify", type, languages: languages.join(","), category }, { method: "post" })
    }
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogPopup>
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Pick the kind of book this project is about. Scriptural types ask for one or more source
                        languages; commentary, biography, and review ask for a category.
                    </DialogDescription>
                </DialogHeader>
                <DialogPanel className="flex flex-col gap-4">
                        ...
                        <Select
                            ...
                            onValueChange={next => {
                                ...
                                if (!(SCRIPTURAL_TYPES as readonly string[]).includes(nextType)) {
                                    ...
                                    setLanguages(selected => selected.filter(language => allowed.includes(language)))
                                }
                            ...
                            <SelectTrigger className="w-full" id="classify-type">
                                <SelectValue>{(value: BookType | "") => <TypeLabel type={value} />}</SelectValue>
                            </SelectTrigger>
                            <SelectPopup>
                                <SelectItem value="">
                    ...
                    {needsLanguage && (
                        <div className="flex flex-col gap-2">
                            ...
                            <Combobox
                                ...
                                onValueChange={next =>
                                    setLanguages((next as { value: LanguageType }[]).map(item => item.value))
                                }>
                                <ComboboxChips>
                                    <ComboboxValue>
                                        {(value: { value: LanguageType; label: string }[]) => (
                                            <>
                                                ...
                                                <ComboboxChipsInput
                                                    ...
                                                    placeholder={value.length > 0 ? undefined : "Pick languages…"}
                                                />
                                            </>
                                        )}
                                    </ComboboxValue>
                    ...
                    {needsCategory && (
                        <div className="flex flex-col gap-2">
                            ...
                            <Select
                                value={category}
                                onValueChange={next => setCategory(String(next ?? "") as CategoryType | "")}>
                                <SelectTrigger className="w-full" id="classify-category">
                                    <SelectValue placeholder="Pick a category…">
                                        {(value: CategoryType | "") =>
                                            value ? <IconLabel icon={Landmark} text={value} /> : "Pick a category…"
                                        }
                                    </SelectValue>
                                </SelectTrigger>
                                <SelectPopup>
