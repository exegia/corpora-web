...
export default function Row({ document }: RowProps) {
    ...
    return (
        <TableRow className="group/row">
                ...
                <div className="flex items-center gap-3">
                    {fileIcon ? (
                        ...
                        <Avatar className="size-8 shrink-0">
                            <AvatarFallback>{document.name.charAt(0).toUpperCase()}</AvatarFallback>
                        </Avatar>
                    )}
            ...
            <TableCell className="text-sm tabular-nums">{document.docsCount?.toLocaleString("en-US") ?? "—"}</TableCell>
            <TableCell className="text-sm tabular-nums">{formatSize(document.sizeBytes)}</TableCell>
            ...
            background above the link overlay and chops the focus ring. */}
                <div className="relative z-10 grid items-center justify-items-end">
                    ...
                    />
                    <div className="flex items-center opacity-0 transition-opacity [grid-area:1/1] group-focus-within/row:opacity-100 group-hover/row:opacity-100">
