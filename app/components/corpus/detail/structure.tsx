...
function NodeRow({ node, depth, archive }: { node: StructureNode; depth: number; archive: CorpusArchive | null }) {
    const expandable = (node.childCount ?? 0) > 0 || (node.children?.length ?? 0) > 0
    ...
    const row = (
        <div
            ...
            <div className="flex min-w-0 items-center gap-2">
                {expandable ? (
                    <CollapsibleTrigger
                        ...
                        <span className="truncate text-xs text-muted-foreground">{node.label}</span>
                    </CollapsibleTrigger>
                ) : (
            ...
            <span className="text-right text-sm tabular-nums">{formatCount(node.childCount)}</span>
        </div>
    )
        ...
        <Collapsible onOpenChange={onOpenChange} open={open}>
            ...
            <CollapsiblePanel>
                {loading ? (
                    ...
                    children.map(child => <NodeRow archive={archive} depth={depth + 1} key={child.id} node={child} />)
                )}
            </CollapsiblePanel>
        </Collapsible>
    )
...
async function loadChildren(node: StructureNode, archive: CorpusArchive | null): Promise<StructureNode[]> {
    ...
    if (archive && node.ref) {
        ...
        return page.items.map(item => Corpus.Explore.structureNodeFromSection(item, node.type))
    }
    return []
}

/** Structure tab: collapsible instance tree, collapsed until a parent is opened. */
export default function Structure({ document, archive }: { document: CorpusDocument; archive: CorpusArchive | null }) {
    const root = useMemo(
        ...
        return (
            <Empty className="py-10">
                <EmptyHeader>
                    ...
                    <EmptyDescription>No live archive is available for this corpus yet.</EmptyDescription>
                </EmptyHeader>
            </Empty>
        )
    }
    ...
    return (
        <Panel bodyClassName="p-0" title="Document hierarchy">
            <div className="grid grid-cols-[minmax(0,1fr)_7rem] gap-3 border-b px-4 py-2 text-xs tracking-wider text-muted-foreground uppercase">
