...
-- ---------------------------------------------------------------------------
-- users (seeded directory; read-only to the app until corpora-auth ships)
-- ---------------------------------------------------------------------------
...
insert into public.users (id, name, username, email) values
...
create table public.project_licenses (
  ...
  agreed_by_user_id uuid not null references public.users (id) on delete restrict,
